...
class TransformerEncoderLayer_QKV(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, res_dropout=0.1, activ_dropout=0.1, activation='gelu'):
        ...
        self.self_attn = nn.MultiheadAttention(embed_dim=embed_dim,num_heads=num_heads,dropout=attn_dropout)#torch自带的多头注意力机制函数
        ...
        if activation=='gelu':
            self.activ=nn.GELU()
        self.layer_norms = nn.ModuleList([nn.LayerNorm(embed_dim) for _ in range(2)])#2层LN
    ...
    def forward(self, x_q, x_k=None, key_len=None):
        ...
        if x_k is None:
            ...
            x_q, _ = self.self_attn(query=x_q, key=x_k, value=x_k, key_padding_mask=key_padding_mask)#虽然是x_q，但其特征为x_v的属性
...
class TransformerEncoder(nn.Module):#模型使用的注意力机制
    ...
    def forward(self, x_q, x_k=None, key_len=None):
        ...
        Args:
            x_q (Tensor): input to the layer of shape (seq_len, batch, embed_dim)
            x_k (None or Tensor): if tensor, input to the layer of shape (seq_len', batch, embed_dim)
            key_len (None or Tensor): if Tensor, input to the layer of shape (batch,)#可以处理不同length的feature
        Returns:
            out shape (seq_len, batch, embed_dim)
        """
        x_q = self.pos_encoder(self.embed_scale*x_q)
        if x_k is not None:  # 自注意力
            x_k = self.pos_encoder(self.embed_scale*x_k)
         # encoder layers
        intermediates = [x_q]#中间层的特征，暂时无用
...
class TransformerEncoder_nopos(nn.Module):#无position功能的transformer
...
class TransformerEncoderLayer_localmask(nn.Module):
    """
    Completed TransformerEncoderLayer for input Q K V
    单个多头注意力机制代码
    ...
    """
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, res_dropout=0.1, activ_dropout=0.1, activation='gelu'):
        super(TransformerEncoderLayer_localmask, self).__init__()
    ...
    def _get_local_mask(self, seq_len, device):
        mask = torch.full((seq_len, seq_len), float('-inf'), device=device)
        for i in range(seq_len):
            start = max(0, i - 1)
            end = min(seq_len, i + 2)  # i + 1 + 1 (inclusive)
            mask[i, start:end] = 0  # Set allowed positions to 0
        mask = mask.masked_fill(mask == float('-inf'), float('-inf')).masked_fill(mask == 0, float(0.0))
        return mask.bool()
    ...
    def forward(self, x_q, x_k=None, key_len=None):
        ...
        if x_k is None:
            key_padding_mask = self._get_key_padding_mask(x_q.shape[0], key_len) if key_len is not None else None
            attn_mask = self._get_local_mask(x_q.shape[0], x_q.device) 
            x_q, _ = self.self_attn(query=x_q, key=x_q, value=x_q, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        else:  # 将x_q嵌入到x_k中
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            key_padding_mask = self._get_key_padding_mask(x_k.shape[0], key_len) if key_len is not None else None
            attn_mask = self._get_local_mask(x_k.shape[0], x_k.device) 
            x_q, _ = self.self_attn(query=x_q, key=x_k, value=x_k, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
...
class TransformerEncoder_localmask(nn.Module):  # 模型使用的注意力机制
    def __init__(self, embed_dim, pos_flag='learned', pos_dropout=0.1, num_heads=4, attn_dropout=0.1, res_dropout=0.1, activ_dropout=0.1, activation='gelu', num_layers=6):
        super().__init__()
        self.embed_dim = embed_dim
        self.pos_flag = pos_flag
        if pos_flag == 'sincos':
            ...
            self.embed_scale = 1.0
            self.pos_encoder = PositionalEncodingLearned1D(embed_dim, pos_dropout)
        elif pos_flag == 'poscnn':
            self.embed_scale = 1.0
            self.pos_encoder = PosCNN(embed_dim, embed_dim, 1, pos_dropout)
        elif pos_flag == 'relative':
            self.embed_scale = 1.0
            self.pos_encoder = RPEAttention(dim=embed_dim, num_heads=8, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.)

        self.layers = nn.ModuleList([])
        for layer in range(num_layers):
            new_layer = TransformerEncoderLayer_localmask(embed_dim,
                                                    num_heads=num_heads,
                                                    attn_dropout=attn_dropout,
                                                    res_dropout=res_dropout,
                                                    activ_dropout=activ_dropout,
